from xlstm_solace_torch.mad.data.instances import (
...
from xlstm_solace_torch.mad.model import layers
from xlstm_solace_torch.mad import model

# Import xLSTM blocks
from xlstm_solace_torch.mad.model.layers import MLSTMBlock, GatedFFN

# Import MAD blocks (chunkwise implementations with multi-backend support)
from xlstm_solace_torch.mad.blocks.mlstm_torch import ChunkwiseMLSTM
try:
    from xlstm_solace_torch.mad.blocks.mlstm_mlx import ChunkwiseMLSTMMLX
    _has_mlx = True
except ImportError:
    _has_mlx = False
    ChunkwiseMLSTMMLX = None
...
layer_registry = {
    ...
    # xLSTM sequence mixers:
    'mlstm': {
        'module': MLSTMBlock,
        'cfg': 'configs/layers/mlstm.yml',
        'shorthand': 'mL'
    },
    # MAD chunkwise mLSTM (multi-backend with PyTorch native and MLX stream chaining)
    'chunkwise-mlstm': {
        'module': ChunkwiseMLSTM,  # Default to PyTorch
        'backends': {
            'pytorch': ChunkwiseMLSTM,
            'mlx': ChunkwiseMLSTMMLX if _has_mlx else None,
        },
        'cfg': 'configs/layers/chunkwise-mlstm.yml',
        'shorthand': 'cL'
    },
    # xLSTM channel mixers:
    'xlstm-gated-ffn': {
        'module': GatedFFN,
        'cfg': 'configs/layers/xlstm-gated-ffn.yml',
        'shorthand': 'xF'
    }
}
