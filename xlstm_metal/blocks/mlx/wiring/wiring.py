...
class MADWiring:
    ...
    def _instantiate_block(self, spec: BlockSpec) -> nn.Module:
        ...
        if spec.block_type == BlockType.MLSTM:
            ...
            return RMSNorm(num_features=d_model, eps=eps, force_float32_reductions=force_float32)
        else:
...
class WiredMADModel(nn.Module):
    ...
    def __init__(
        ...
        # In MLX, we can just set blocks as attributes and MLX will track them
        self.blocks = {}
        for name in wiring.block_names:
            block = wiring.get_block(name)
            self.blocks[name] = block
            # Set as attribute for MLX parameter tracking
            setattr(self, f"_block_{name}", block)
    ...
    def __call__(
        ...
        for stage_idx, stage_blocks in enumerate(self.stages):
            ...
            for block_name in stage_blocks:
                ...
                # Check if block expects hidden states (xLSTM blocks do, others don't)
                spec = self.wiring.block_specs[block_name]
                if spec.block_type == BlockType.MLSTM:
                    # xLSTM blocks return (output, new_hidden)
                    output, new_hidden = block(block_input, block_hidden)
                    new_hidden_states[block_name] = new_hidden
                else:
                    # Other blocks (Embedding, Linear, Norm) just return output
                    output = block(block_input)
                    new_hidden_states[block_name] = None
