...
# Import Metal kernels if available
try:
    from ...kernels.mlstm_chunkwise_kernel import (
...
class mLSTMCell(nn.Module):
    ...
    def __call__(
        ...
        if self.kernel_backend == "metal_parallel" and METAL_KERNELS_AVAILABLE:
            h, C_new, n_new, m_new = self._apply_metal_parallel_kernel(
                q, k, v, i_gate, f_gate, C, n, m
            )
        elif self.kernel_backend == "metal_sequential" and METAL_KERNELS_AVAILABLE:
