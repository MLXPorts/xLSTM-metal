...
if TYPE_CHECKING:
    from xlstm_metal.blocks.mlx.wiring import WiredMADModel
...
def load_safetensors_into_wiring(model_dir: str, wiring: "MADWiring"):
    """
    Load xLSTM weights directly from HuggingFace safetensors into MADWiring neural circuit.
    ...
    Args:
        model_dir: Path to HuggingFace model directory with safetensors
        wiring: MADWiring neural circuit to load weights into
    """
    model_dir = Path(model_dir)
    ...
    # Load embedding
    if 'backbone.embeddings.weight' in hf_weights and 'embedding' in wiring.neurons:
        wiring.neurons['embedding'].weight = mx.array(hf_weights['backbone.embeddings.weight'])
        print(f"✓ Loaded embedding: {hf_weights['backbone.embeddings.weight'].shape}")

    # Load blocks
    num_blocks = sum(1 for name in wiring.neurons.keys() if name.startswith('xlstm_'))
    print(f"Loading {num_blocks} xLSTM blocks...")

    for i in range(num_blocks):
        block_name = f'xlstm_{i}'
        if block_name not in wiring.neurons:
            print(f"Warning: {block_name} not found in wiring")
            continue

        block = wiring.neurons[block_name]
        hf_prefix = f'backbone.blocks.{i}'
    ...
    # Load output norm
    if 'backbone.out_norm.weight' in hf_weights and 'out_norm' in wiring.neurons:
        wiring.neurons['out_norm'].weight = mx.array(hf_weights['backbone.out_norm.weight'])
        print(f"✓ Loaded out_norm: {hf_weights['backbone.out_norm.weight'].shape}")

    # Load LM head
    if 'lm_head.weight' in hf_weights and 'lm_head' in wiring.neurons:
        wiring.neurons['lm_head'].weight = mx.array(hf_weights['lm_head.weight'])
        print(f"✓ Loaded lm_head: {hf_weights['lm_head.weight'].shape}")

    print(f"\n✅ Successfully loaded all pretrained weights into neural circuit")
