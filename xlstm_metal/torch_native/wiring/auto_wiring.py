"""Automatic wiring generation from safetensors structure (PyTorch backend).
...
class AutoWiring(Wiring):
    ...
    def create_block_cell(self, block_idx: int, **kwargs):
        ...
        if block_type == 'mlstm':
            from xlstm_metal.torch_native.blocks.mlstm.mlstm_block import mLSTMBlock
            return mLSTMBlock.from_config(block_idx, self.config, **kwargs)
        elif block_type == 'slstm':
            raise NotImplementedError("sLSTM block not yet ported to torch backend")
        elif block_type == 'attention':
            raise NotImplementedError("Attention block not yet implemented for torch backend")
        else:
...
def create_auto_wiring(model_dir: str, config: Optional[Dict] = None) -> AutoWiring:
    ...
    if config is None:
        from xlstm_metal.torch_native.utils.config_loader import load_config
