...
Port the mLSTM chunkwise forward kernels from Triton (CUDA/ROCm) to Metal C++ to enable high-performance inference on
Apple Silicon (M3 Ultra with 128 GPU cores).
...
**Lesson**: Port line-by-line, keeping all computations even if they seem redundant. The Triton kernel's structure is
carefully designed for correctness.
...
The Metal kernels enable efficient xLSTM-7B inference on Apple Silicon, providing a major performance improvement over
the O(T) sequential baseline.
