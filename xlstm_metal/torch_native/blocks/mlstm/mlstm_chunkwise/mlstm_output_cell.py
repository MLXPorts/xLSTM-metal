...
from xlstm_metal.torch_native.blocks.rms_norm import MultiHeadRMSNorm
...
class mLSTMOutputCell(nn.Module):
    ...
    def forward(
        ...
        o_gate = torch.sigmoid(self.ogate_proj(x_orig))  # [B, S, NH*DH_v]

        # Gate the normalized hidden states
        h_gated = torch.multiply(h_norm, o_gate)
