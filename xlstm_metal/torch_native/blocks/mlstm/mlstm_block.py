...
from xlstm_metal.torch_native.blocks.mlstm.mlstm_chunkwise.mlstm_neuron import mLSTMNeuron
from xlstm_metal.torch_native.blocks.rms_norm import RMSNormCell
from xlstm_metal.torch_native.utils import resolve_dtype
...
class mLSTMBlock(nn.Module):
    ...
    def forward(
        ...
        Architecture:
            residual = x
            x = norm_mlstm(x)
            x, new_state = mlstm_cell(x, state)
            x = x + residual
