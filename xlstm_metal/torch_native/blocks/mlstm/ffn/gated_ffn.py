"""Torch implementation of the GatedFFN recurrent-style module.
...
class GatedFFN(nn.Module):
    ...
    def _apply_fc(self, tensor: torch.Tensor) -> torch.Tensor:
    ...
    def forward(
        ...
        if not is_batched:
            inputs = torch.unsqueeze(inputs, dim=batch_dim)

        # FFN is stateless, so no hidden state initialization needed
        h_state = None

        outputs = []
        for t in range(inputs.shape[seq_dim]):
            if self.batch_first:
                step_input = inputs[:, t, :]
            else:
                step_input = inputs[t, :, :]
        ...
        # Prepare output
        if self.return_sequences:
            stacked = torch.stack(outputs, dim=seq_dim)
            readout = stacked
        else:
            # h_out is defined in the loop, and will be the last output
            readout = self._apply_fc(h_out)

        # Remove batch dimension if input wasn't batched
        if not is_batched:
            readout = torch.squeeze(readout, dim=batch_dim)
