...
from kernel_development.mlstm_block.mlstm_chunkwise.vendored.triton.xl_chunk.bw_kernel_parallel_dQ import mlstm_chunkwise__parallel_bw_dQ_kernel
...
def mlstm_chunkwise__parallel_bw_dQ(
    ...
    assert siz_b_LQ % siz_b_LKV == 0, "siz_b_LKV must be divisible by siz_b_LQ"
    ...
    matDeltaQ = jax.ShapeDtypeStruct(shape=(B, NH, S, DHQK), dtype=output_dtype)

    grid = (num_b_DHQK, num_b_LQ, NC * B * NH)
    matDeltaQ = jt.triton_call(
        matQ,
        matK,
        matV,
        vecI,
