...
from kernel_development.torch_msltm_kernels.jax.stride_utils import get_stride
from kernel_development.torch_msltm_kernels.jax.utils import jax2triton_dtype
from kernel_development.mlstm_block.mlstm_chunkwise.vendored.triton.xl_chunk.bw_kernel_parallel_dV import mlstm_chunkwise__parallel_bw_dV_kernel
...
def mlstm_chunkwise__parallel_bw_dV(
...
) -> jax.Array:  # matDeltaV (B, NH, S, DHHV)
    """
    ...
    matDeltaV = jax.ShapeDtypeStruct(shape=(B, NH, S, DHHV), dtype=output_dtype)

    grid = (num_b_DHHV, num_b_LKV, NC * B * NH)
    matDeltaV = jt.triton_call(
        matQ,
        matK,
        matV,
        vecI,
        ...
        num_warps=num_warps,
        grid=grid,
        kernel=mlstm_chunkwise__parallel_bw_dV_kernel,
    )
