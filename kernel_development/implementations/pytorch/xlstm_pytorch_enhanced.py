...
class sLSTMBlock(nn.Module):
    ...
    def __init__(self, config: xLSTMConfig):
        ...
        self.head_num = config.head_num
        self.hidden_dim = config.head_dim * config.head_num
        p_factor = config.p_factor[1]  # sLSTM factor
    ...
    def forward_step(self, x, hidden_state, use_conv=False):
        ...
        i_t = soft_cap(self.W_i(x_c) + self.R_i(h_tm1), self.config.gate_soft_cap)
        f_t = soft_cap(self.W_f(x_c) + self.R_f(h_tm1), self.config.gate_soft_cap)
...
class xLSTM(nn.Module):
    """Enhanced xLSTM model combining sLSTM and mLSTM blocks"""
    def __init__(self, config: xLSTMConfig):
        super().__init__()
        self.config = config
