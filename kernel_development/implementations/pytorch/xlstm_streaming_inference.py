...
class StreamingmLSTMBlock(nn.Module):
    ...
    def forward_streaming(self, x: torch.Tensor, hidden_state: Dict[str, torch.Tensor]) -> Tuple[
...
class StreamingxLSTM(nn.Module):
    ...
    @torch.no_grad()
    def stream_forward(self, token_id: torch.Tensor, states: List[Dict[str, torch.Tensor]]) -> Tuple[
