...
def setup_data():
    ...
    tensors.update(dict(B=B,NH=NH,S=S,DHQK=DHQK,DHHV=DHHV,NC=NC,L=L,qk_scale=qk_scale,save_states_every_nth_chunk=save_states_every_nth_chunk))
    return tensors
...
def test_bw_recurrent_kernel(setup_data):
    d = setup_data
    out = mlstm_chunkwise_recurrent_bw_dC_metal(d['matQ'], d['vecF'], d['scaM_inter'], d['vecM_combine'], d['matDeltaH'], d['vecN_out'], d['matDeltaC_last'], d['NC'], d['L'], d['qk_scale'], save_states_every_nth_chunk=d['save_states_every_nth_chunk'])
    print("bw_recurrent sum", float(mx.sum(out)))
    assert mx.sum(out).item() != 0

def test_bw_parallel_dK_kernel(setup_data):
    d = setup_data
    matDeltaK = mlstm_chunkwise_parallel_bw_dK_metal(d['matQ'], d['matK'], d['matV'], d['vecI'], d['vecB'], d['vecA'], d['matCstate_all'], d['vecNstate_all'], d['scaMstate_all'], d['vecN_out'], d['vecM_out'], d['matDeltaH_out'], d['matDeltaC_states'], d['NC'], d['L'], d['qk_scale'])
    print("bw_dK sum", float(mx.sum(matDeltaK)))
    assert mx.sum(matDeltaK).item() != 0

def test_bw_parallel_dQ_kernel(setup_data):
    d = setup_data
    matDeltaQ = mlstm_chunkwise_parallel_bw_dQ_metal(d['matQ'], d['matK'], d['matV'], d['vecI'], d['vecB'], d['vecA'], d['matCstate_all'], d['vecNstate_all'], d['scaMstate_all'], d['vecN_out'], d['vecM_out'], d['matDeltaH_out'], d['matDeltaC_states'], d['NC'], d['L'], d['qk_scale'])
    print("bw_dQ sum", float(mx.sum(matDeltaQ)))
    assert mx.sum(matDeltaQ).item() != 0

def test_bw_parallel_dV_kernel(setup_data):
    d = setup_data
    matDeltaV = mlstm_chunkwise_parallel_bw_dV_metal(d['matQ'], d['matK'], d['matV'], d['vecI'], d['vecB'], d['vecA'], d['matCstate_all'], d['vecNstate_all'], d['scaMstate_all'], d['vecN_out'], d['vecM_out'], d['matDeltaH_out'], d['matDeltaC_states'], d['NC'], d['L'], d['qk_scale'])
    print("bw_dV sum", float(mx.sum(matDeltaV)))
    assert mx.sum(matDeltaV).item() != 0

def test_fw_parallel_kernel(setup_data):
    d = setup_data
    matHout, vecNout, vecMout = mlstm_chunkwise_parallel_fw_Hintra_metal(d['matQ'], d['matK'], d['matV'], d['matCstate_all'], d['vecNstate_all'], d['scaMstate_all'], d['vecI'], d['vecB'], d['NC'], d['L'], d['qk_scale'])
